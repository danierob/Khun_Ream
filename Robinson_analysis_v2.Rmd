...
ggplot(data = LAI_plots, aes(x = date, y = LAI_plot, color = plot_type)) +
  ...
  labs(title = "Seasonal Variation in Canopy Cover by Plot Type",
       x = "Date", y = "Canopy Cover (LAI)", color = "Plot Type")
...
m2 <- lm(grass_mass ~ mean_LAI, data=data_avg) 

ms2 <- segmented(m2, npsi = 2)
plot(ms2)
summary(ms2)
confint.segmented(ms2)
...
pred_df$grass_mass_pred <- predict(ms2, newdata = pred_df)
...
ggplot(data_avg, aes(x = mean_LAI, y = grass_mass)) +
  ...
  theme_minimal() +
  geom_line(data = pred_df, aes(x = mean_LAI, y = grass_mass_pred), color = "red"
  ) 
...
```
Going to take a peak at Litter as a squared (2 order poly graph)
```{r litter squared}
#random intercepts model
...
bulk_seasons <- bulk_seasons %>%
  mutate(bulk_density = ifelse(grass_mass > 240, (grass_mass+litter_mass+wood_mass+other_mass)/
                                 (grass_height/100),((grass_mass+ litter_mass + wood_mass + other_mass)/
...
ggplot(bulk_seasons, aes(x = mean_LAI, y = bulk_density/1000, color = season))+
  ...
  labs(x = "Canopy Cover (LAI)", y = "Fine Fuel Bulk Density (g/m3)")+
  theme_bw() +
  theme(text=element_text(size= 16), legend.position="none", 
                 plot.title = element_text(hjust = 0.5))
...
ggplot(kestrel_wind, aes(x = date, y = wind_speed, color = plot_id)) +
  ...
  labs(title = "Daily Wind Speed at Different Canopy Cover (LAI) Across Dry Season",
       x = element_blank(), y = "Wind Speed (m/s)", color = "LAI") +
  scale_color_manual(labels = c("1.7", "1.6", "0.7", "4.7", "5.4", "6.0"), values = c("red", "red4", "darkorange", "blue", "purple", "purple4")) + 
  theme(text=element_text(size= 16), legend.position="none", 
             plot.title = element_text(hjust = 0.5))+
  theme_minimal()
...
```{r wind model}
m_wind <- lmer(wind_speed ~ plot_type * month + (1 | plot_id), data = kestrel_wind)
...
emmeans(m_wind, pairwise ~ plot_type | month)
```
...
p1 <- ggplot(litter, aes(x = LAI, y = litter, color = season)) +
  ...
  scale_color_manual(values = c("early" = "orange", "late" = "darkred")) +
...
I'm still going to average LAI around the observations (e.g. early season behaveplus outputs will relate to the3 average of LAI obs 2, 3, and 4) to account for any missing LAI values and temper out the differences in light angles, measurement errors, etc.
...
write.csv(behave_tbl_3.2,"C:/Users/d/Downloads/behave_table_V3.csv")
...
### 3. 1) Modelled fire effects across the canopy gradient at the early and late dry season.
...
Since we are less concerned with precisely interpreting differences between factors than we are with the fitting the precise relationship between our modeled data and canopy cover, we will use GAMMs so simply detect the presence or absence of a threshold.

**b) How do fire effects vary by season in either forest type?**
We will be isolating either side of the threshold by only considering the plots of the same type. This will allow for more direct and interpretable linear comparisons of before and after and a more detailed assessment of how risk of fire relatively changes on either side of the boundary. For the open-canopy, fire danger should increase quite a lot in terms of total ROS and intensity, but I believe the relative change should be greater on the closed canopy side as the litter fm and litter loads start to permit late season fire in the closed system. 


PRESENT AS A GAMM BUT RUN THE ACTUAL STATISTICAL ANALYSES ON THE SUBSETS OF OPEN VS CLOSED??
#### a) How do modelled fire effects and PIG change across the canopy gradient?
```{r ROS data analysis}

# Creating a GAM model with a random intercept for 'plot_id'
gamm_model<- gam(ROS ~ season + s(mean_LAI, by = season) + s(plot_id, bs = "re"),
                       data = behave, method = "REML")
...
gamm_model_tw<- gam(ROS ~ season + s(mean_LAI, by = season) + s(plot_id, bs = "re"),
                family = tw(link = "log"), data = behave, method = "REML")
...
``` {r ROS plot}
#PLOT
gam_data <- expand.grid(mean_LAI = seq(min(behave$mean_LAI), max(behave$mean_LAI), length = 100),season = unique(behave$season), plot_id = unique(behave$plot_id))

# Make predictions using the GAM model
gam_data$predicted <- predict(gamm_model_tw, newdata = gam_data, type = "response")

# Create a plot using predicitons from GAM
ggplot(gam_data, aes(x = mean_LAI, y = predicted, color = season)) +
  geom_line() +
  geom_point(data = behave, aes(x = mean_LAI, y = ROS, color = season), alpha = 0.5) +
   labs(title = "Modelled Rate of Fire Spread (ROS) Across Canopy Cover",
       x = "Canopy Cover (LAI)", y = "Rate of Spread (m/min)", color = "Season") +
  scale_color_manual(labels = c("Early", "Late"), values = c("orange", "darkred"))+
  theme_minimal()
...
#PLOT
PIG_data <- expand.grid(mean_LAI = seq(min(behave$mean_LAI), max(behave$mean_LAI), length = 100),
                        season = unique(behave$season))
...
ggplot(PIG_data, aes(x = mean_LAI, y = predicted, color = season)) +
  geom_line() +
  geom_point(data = behave, aes(x = mean_LAI, y = PIG, color = season), alpha = 0.5) +
       labs(title = "Modelled Probability of Ignition Across Canopy Cover",
       x = "Canopy Cover (LAI)", y = "Probability of Ignition (%)", color = "Season") +
  scale_color_manual(labels = c("Early", "Late"), values = c("orange", "darkred"))+
  theme_minimal()
...
To compare early and late season fire effects we will isolate data by forest type 
(open or closed) and then run linear mixed models for the relationships between LAI and fire effects for either season.
...
ggplot(closed_data, aes(x = mean_LAI, y = ROS, color = season))+
  geom_point(alpha = .5) + geom_smooth(method = "lm", fill = "NA")+
  labs(title = "Closed Canopy ROS",
