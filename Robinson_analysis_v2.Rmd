...
###1. 5) Fine Fuel Distribution by Fuel Type and by season
...
```{r fine fuel composition summaries}
fuel_summaries <- fuel_seasons %>% 
  group_by(plot_type, season) %>% 
  summarize(LAI = mean(LAI), grass_mass = mean(grass_mass), litter_mass = mean(litter_mass), wood_mass = mean(wood_mass), seedlings = mean(seedlings), fuel_load = mean(fuel_load), fine_load = mean(fine_load),
            fuel_depth = mean(fuel_depth), bulk_density = mean(bulk_density))
...
segmented_model_early <- segmented(model_early, seg.Z = ~mean_LAI, psi = c(1.8,2.4), data = behave_early)
segmented_model_late <- segmented(model_late, seg.Z = ~mean_LAI,  psi = c(1.8, 2.4), data = behave_late)
...
ggplot(behave, aes(x = mean_LAI, y = ROS)) +
  ...
  theme_minimal() +
  geom_line(data = pred_df1, aes(x = mean_LAI, y = ROS_pred), color = "darkred", size = .8) +
  geom_line(data = pred_df, aes(x = mean_LAI, y = ROS_pred), color = "blue4", size = .8) +
  ylim(-0.1,30)
