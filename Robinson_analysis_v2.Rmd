...
mnolog <- lmer(litter ~ LAI*season + (1|plot_id), data = litter, REML = TRUE)
...
pred_data <- data.frame(
  LAI = seq(min(litter$LAI), max(litter$LAI), length.out = 100),
  season = rep(c("early", "late"), each = 100))

pred_data$pred_log_litter <- predict(m1, newdata = pred_data, re.form = NA)

# Reverse transform the predicted values
pred_data$pred_litter <- exp(pred_data$pred_log_litter)
...
scale_color_manual(values = c("early" = "blue4", "late" = "darkred")) +
      labs(title = "Litter", x = "Canopy Cover(LAI)", y = "Litter Fuel Mositure Content (%)", color = element_blank())+ 
   theme_bw()
...
p1 <- ggplot(litter, aes(x = LAI, y = log(litter), color = season)) +
