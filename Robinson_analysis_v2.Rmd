...
pred_df <- data.frame(mean_LAI = seq(min(data$mean_LAI), max(data$mean_LAI), length.out = 100))
...
#Here we are determining fuel depth as grass height if there is more that 240g/m2 of grass, 
#and as the weighted mean of grass height and litter depth otherwise.

data_bulk <- data_bulk %>%
  mutate(bulk_density = ifelse(grass_mass > 240, (grass_mass+litter_mass+wood_mass+other_mass)/
                                 (grass_height/100),((grass_mass+ litter_mass + wood_mass + other_mass)/((grass_height*grass_ratio+litter_depth*litter_ratio)/100))))
...
ggplot(closed_data, aes(x = mean_LAI, y = ROS, color = season))+
  ...
  labs(title = "Closed Canopy Modelled PIG Across Canopy Cover",
       x = "Canopy Cover (LAI)", y = "Probability of Ignition (%)", color = "Season") +
  scale_color_manual(labels = c("Early", "Late"), values = c("orange", "darkred"))+
  theme_minimal()
