...
biomass_seasons <- biomass %>%
  group_by(plot_id, quad_id, season) %>% 
  summarise(grass_mass = mean(mass_grass*4), litter_mass = mean(mass_litter*4), wood_mass = mean(mass_wood*4), woody_mass = mean((mass_liana + seedlings*10)*4), grass_height = mean(grass_height), litter_depth = mean(litter_depth), grass_per = mean(grass_per), litter_per = mean(litter_per))
...
gamm_model_tw <- gam(grass_mass ~ season + s(mean_LAI, by = season) + s(plot_id, bs = "re"),
...
gam_model <- gam(litter_mass ~ season + s(mean_LAI, by = season), 
           data = LAI_earlyxbiomass_seasons, method = "REML")
...
```
```{r GAM tw}
gam_model_tw <- gam(litter_mass ~ season + s(mean_LAI, by = season),
                    family = tw(link = "log"), data = LAI_earlyxbiomass_seasons, method = "REML")
...
# Creating a GAM model
gam_modelb <- gam(litter_mass ~ season + s(mean_LAI, by = season, k = 20),
...
**1.2.4a)**
```{r 1.2.4a analysis}
model <- lm(bulk_density ~ mean_LAI*season.y, data = LAI_earlyxbiomass_bulk)
summary(model)
```

```{r 1.2.4a plot}
ggplot(LAI_earlyxbiomass_bulk, aes(x = mean_LAI, y = bulk_density, color = season.y))+
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
...
ggplot(gam_data, aes(x = mean_LAI, y = predicted, color = season)) +
  geom_line() +
  geom_point(data = gam_data, aes(x = mean_LAI, y = predicted, color = season), alpha = 0.5) +
  labs(title = "Relationship between canopy cover and ROS by season",
       x = "LAI",
       y = "Rate of Spread (ROS)") +
  theme_minimal()
...
ggplot(gamm_data, aes(x = mean_LAI, y = predicted, color = season)) +
  geom_smooth()+
  geom_point(data = gamm_data, aes(x = mean_LAI, y = predicted, color = season), alpha = 0.5) +
  labs(title = "Seasonal Difference in ROS and LAI (GAMM)",
       x = "LAI",
       y = "ROS") +
  theme_minimal()
