...
```{r 1.2.2a plot}
ggplot(LAIxbiomass_2, aes(x = mean_LAI, y = litter_mass, color = season)) +
  geom_point() +
...
In Q1 we looked at how fuel is distributed along the canopy gradient, in that question we were essentially looking at fuel availability in terms of the physical presence of flammable vegetation. At the coarsest scale, fuel presence is determined by climate (ref...). In our system, the climate is sufficient to support continuous vegetation and the presence of fuel is largely moderated through feedbacks (Pausas, ref..) ( see outline). When fuel is present, its physical availability tends to change due to relatively slow process (growth and decay), unless it is removed by some outside influence (a grazing cow or a fire). In the absence of cow or fire, surface fuels will accumulate gradually with leaf fall (dry season) or new growth (wet season) or decrease gradually through dessication.

Contrastingly, weather often affects surface fuel on a finer temporal scale (ref). The most direct way that weather affects fuel availability is through precipitation. Wet fuel is not available to fire, no matter how much fuel there is. So after they are saturated, fuels must dry sufficiently before they can be available to fire. The threshold fuel moisture at which combustion can be sustained is referred to as the **moisture of extinction.** The time it takes for a fuel parcel to cross the moisture of extinction and become dry enough for combustion is dependent on several interacting factors ( need to elaborate on this, perhaps, see the fuel moisture section in annotated bib ), most relevant of which is **microclimate buffering** (ref). Fuels that are under a dense canopy take longer to dry that fuels that are in the open exposed to direct sunlight. But the rate at which fuels cure In Q2, we are concerned with this more mercurial side of fuel availability: the components of fuel availability that are determined by **microclimate.**
