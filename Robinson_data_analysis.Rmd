...
In this analysis we will be looking at the relationship between microclimate and canopy cover across the two scales that are most important for understanding the role of microclimate in determining fuel availability: daily scales and seasonal scales. We will use the early season data (as in section 1.1) when canopy cover and surface fuels were at peak biomass, which should coincide with peak microclimate buffering potential -- more shade means less exposure to the elements. Additionally, the early season is the time of year when the majority of Cambodia's grass burns, so looking at this time

#### 2.1.2)

```{r 2.1.2 setup, include = FALSE}
...
mixed.lmer2.1.1 <- lmer(litter ~ mean_LAI + (1|plot_id), data = LAIxfm)
summary(mixed.lmer2.1.1)

```

```{r season average fm plot}
# Extract the prediction data frame
pred <- ggpredict(mixed.lmer2.1.1, terms = c("mean_LAI"))  # this gives overall predictions for the model
...
### 2.2) How does the relationship between microclimate and canopy cover change throughout the dry season?

#### 2.2.1) Fuel moistures by observation
