...
```{r 1.2.1a plot}
ggplot(LAIxbiomass_early_late, aes(x = mean_LAI, y = grass_mass, color = season)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 3), se = FALSE) +
  labs(title = "Late season decreases canopy cover AND grass load, exaggerating the canopy threshold for grass growth",
       x = "LAI", y = "Grass Load (g)", color = "season") +
  ylim(0, 230) +
  theme_minimal()                                                                                                                                                                                                                           
```
...
```{r 1.2.2b plot}
ggplot(LAIxbiomass_early_late, aes(x = mean_LAI, y = litter_mass, color = season)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE) +
  labs(title = "Late season litter loads are greater than early season loads across the canopy gradient",
       x = "LAI", y = "Litter Load (g)", color = "Season") +
...
LAIxfm <- left_join(LAI_early, fm_avg, by = c("quad_id", "plot_id"))
