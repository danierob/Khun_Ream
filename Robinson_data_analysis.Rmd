...
group_by(plot_id, quad_id, obs_period) %>% 
  summarize(soil_temp = mean(soil_temp), surface_temp = mean(surface_temp), air_temp = mean(air_temp), soil_moist = mean(soil_moisture))
...
``` {r 2.1.1 plot}
ggplot(LAIxTOMST2.1, aes(x = LAI, y = air_temp, color = obs_period)) +
