...
# PLOT IT OUT
ggplot(data, aes(x = mean_LAI, y = grass_mass)) +
...
ggplot(gamm_data, aes(x = mean_LAI, y = predicted, color = season)) +
  geom_line()+
  geom_point(data = data, aes(x = mean_LAI, y = grass_mass, color = season), alpha = 0.5) +
  labs(title = "Seasonal Difference in Grass Load (GAMM)",
       x = "LAI",
       y = "grass") +
  geom_hline(yintercept = 180.4, linetype = "dashed") +
  theme_minimal()
...
#### 1.2.4) Changes in fuel bulk density across the dry season
...
bulk <- LAI_earlyxbiomass_seasons %>%
  mutate(bulk_density = ifelse(grass_mass > 10, (grass_mass+litter_mass)/ (grass_height/100), (grass_mass+litter_mass)/ (litter_depth/100)))
...
p <- ggplot(LAIxTOMST2.1, aes(x = LAI, y = max_air_temp)) +
  geom_point() +
  geom_smooth(fill = NA) +
   facet_wrap(~obs_period, labeller = labeller(obs_period = facet_labels))+
  labs(x = "LAI", y = "Max Daily Temp (Celsius)",
       title = "Surface Microclimate Temps Across LAI") +
  theme_bw()

(p + facet_grid(cols = vars(obs_period),labeller = labeller(obs_period = facet_labels)))
...
model <- lmer(litter ~ mean_LAI + (1|plot_id), data = LAIxfm_avg)
summary(model)
...
(ggplot(pred) + 
   ...
   geom_ribbon(aes(x = x, ymin = predicted - std.error, ymax = predicted + std.error), 
               fill = NA, alpha = 0.5) +  # error band
   geom_point(data = LAIxfm_avg,                # adding the raw data
              aes(x = LAIxfm_avg$mean_LAI, y = (LAIxfm_avg$litter))) + 
...
ggplot(litter2.2, aes(x = LAI, y = litter, color = obs_period)) +
  ...
  scale_color_manual(values = c("1" = "gold", "2" = "gold3", "4" = "red", "5" = "blue", "6" = "green", "7" = "purple"),
...
For each quadrant (n = 46) we will average the fuel moisture contents from the 8 observations throughout the season. I believe this method of averaging all observations rather than using any single fuel moisture observation will provide the best representation of fuel moisture for a given quadrant across the season and will mitigate the effects of deviations in fuel moisture sampling time. Dead fuel moisture content is very sensitive to diurnal fluctuations in microclimate, and maximum daily fuel curing lags behind the sun's rising each day. By averaging fuel moisture across the season, we will reduce the variation attributed to differences in fuel moisture collection time (all measurements were taken after 11:00 am to account for this to some degree, but variation still exists). For BehavePlus input we will use fuel moisture from several fuel categories: live woody, live herbaceous, 100 hour, 10 hour, and 1 hour fuel (for 1 hr we will include litter fuel moistures which are structurally similar to 1 hr sticks).
