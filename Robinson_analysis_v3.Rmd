...
#make ggplot with emmip data
p <- ggplot(data = bulk_dat, aes(x = LAI, y = (yvar^2), color = season)) +
...
write.csv(kestrel,"C:/Users/d/Desktop/UBC 2023/Robinson-Project-MSc/KH_data/Kestrel/kestrel_max.csv", row.names = FALSE)
...
m_rh <- lmer(mean_rh ~ plot_type * season + (1 | plot_id), data = kestrel_midday)
...
ggplot() +
  ...
  geom_line(data = pred_data, aes(x = LAI, y = fit, color = season), size = 1) +
  labs(
    title = "Segmented Mixed-Effects Model with Seasonal Interaction",
    x = "Canopy Cover (LAI)",
...
ms <- segmented.lme(m_fl, ~LAI, psi = 2.5, z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
ms <- segmented.lme(m_int, ~LAI, psi = 2.5, z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
