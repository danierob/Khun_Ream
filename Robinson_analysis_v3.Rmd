...
#add points from the original data
p4 + geom_point(data = bulk_seasons, aes(x = LAI, y = bulk_density, color = season), alpha = .5) +
  theme(
    axis.title = element_text(size = 10),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
...
ms <- segmented.lme(m_ros, ~LAI, z.psi = ~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
# Extract fixed effects
fixed_effects <- fixed.effects(ms)
...
#emmip
pig_dat <- emmip(m_pig, season ~ (LAI), at=mylist , CIs=TRUE, plotit = FALSE)

#make ggplot with emmip data
p4 <- ggplot(data = pig_dat, aes(x = LAI, y = (yvar), color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=UCL, ymin=LCL, fill = season), alpha = .3, size = .3) +
  labs(x="Canopy Cover (LAI)", y = "Probability of Ignition (%)", color = "Season", fill = "Season")+
  theme_bw()+ ylim(0,100)

#add points from the original data
p4 + geom_point(data = behave, aes(x = LAI, y = PIG, color = season), alpha = .5) +
  theme(
    axis.title = element_text(size = 10),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
