...
p1 <- ggplot() +
  geom_point(data = data_season, aes(x = LAI, y = grass_mass, color = season)) +
...
```{r gls tomst temp model}

m0 <- gls(mean_air_temp ~ LAI*season, correlation = corExp(form = ~ mean_air_temp + LAI | plot_id), data = data)
summary(m0)
...
# Extract covariance matrix of the parameters
vcov_matrix <- vcov(m0)

# Calculate the linear predictor (Xb)
X <- model.matrix(~ LAI * season, data = new_data)  # Adjust according to your model
linear_pred <- X %*% params

# Calculate standard errors
# Compute the variance-covariance matrix of the predictions
var_pred <- diag(X %*% vcov_matrix %*% t(X))
std_errors <- sqrt(var_pred)

# Calculate confidence intervals
z_score <- qnorm(0.975)  # For 95% CI
lower_ci <- linear_pred - z_score * std_errors
upper_ci <- linear_pred + z_score * std_errors
...
ggplot(data = data, aes(x = LAI, y = mean_air_temp, color = season)) +
  geom_point() +  # Add points
  geom_line(data = new_data, aes(x = LAI, y = predicted, color = season), size = 1) +  # Add regression lines
  geom_ribbon(data = new_data, aes(x = LAI, y = predicted, ymin = lower_ci, ymax = upper_ci, fill = season), alpha = 0.2) +  
  labs(x = "Leaf Area Index (LAI)",
       y = "Mean Air Temperature",
       color = "Season",
       fill = "Season") +  # Add labels
  theme_minimal()  # Use a minimal theme

```

```{r lm and lmer plots and transformations}

m_lm <- lm((mean_air_temp) ~ LAI*season, data = data)
summary(m_lm)

m1 <- lmer((mean_air_temp) ~ LAI*season + (LAI | plot_id), data = data, REML=TRUE)
summary(m1)

#add log and sqrt data
data <- data %>% 
group_by(mean_air_temp, plot_id, LAI, quad_id) %>%
mutate(ln_temp = log(mean_air_temp)) %>% 
mutate(sqrt_temp = sqrt(mean_air_temp))

m2 <- lmer(sqrt_temp ~ LAI*season + (1|plot_id), data = data, REML=TRUE)

m3 <- lmer(ln_temp ~ (LAI)*season +(1|plot_id), data = data, REML=TRUE)
...
#add points from the original data
p + geom_point(data = LAI_avgxTOMST_seasons, aes(x = LAI, y = mean_air_temp, color = season), alpha = .5) 


##LMER MODEL
#emmip
temp_dat <- emmip(m1, season ~ LAI, at=mylist , CIs=TRUE, plotit = FALSE)
...
##SQUARED MODEL
#emmip
temp_dat <- emmip(m2, season ~ LAI, at=mylist , CIs=TRUE, plotit = FALSE)

#make ggplot with emmip data
p <- ggplot(data = temp_dat, aes(x = LAI, y = yvar^2, color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=UCL^2, ymin=LCL^2, fill = season), alpha = .3, size = .3) +
  labs(x="Canopy Cover (LAI)", y="Temp", color = "Season", fill = "Season")+
  theme_bw()

#add points from the original data
p + geom_point(data = data, aes(x = LAI, y = mean_air_temp, color = season), alpha = .5) 


##LOG MODEL
#emmip
temp_dat <- emmip(m3, season ~ LAI, at=mylist , CIs=TRUE, plotit = FALSE)

#make ggplot with emmip data
p <- ggplot(data = temp_dat, aes(x = LAI, y = exp(yvar), color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=exp(UCL), ymin=exp(LCL), fill = season), alpha = .3, size = .3) +
  labs(x="Canopy Cover (LAI)", y="Temp", color = "Season", fill = "Season")+
  theme_bw()
...
behave <- read_csv("C:/Users/d/Desktop/UBC 2023/Robinson-Project-MSc/KH_data/Behaveplus/behave_seasons_V6.csv", 
     col_types = cols(LAI = col_number(), 
         ...
         flame_length = col_number(), PIG = col_number(), grass_load = col_number()))
...
ms2 <- segmented.lme(m_fl, ~LAI, psi = 2.5, z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
ms3 <- segmented.lme(m_int, ~LAI, psi = 2.5, z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
#add points from the original data
p4 <- p4 + geom_point(data = behave, aes(x = LAI, y = PIG, color = season), alpha = .5) +
  theme(
    axis.title = element_text(size = 14),   
    legend.title = element_text(size = 12),     
    legend.text = element_text(size = 10),        
