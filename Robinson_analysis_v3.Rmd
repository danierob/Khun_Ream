...
m1 <- lm(grass_mass ~ mean_LAI, data=data_grass) 
...
pred_df <- data.frame(mean_LAI = seq(min(data_grass$mean_LAI), max(data_grass$mean_LAI), length.out = 100))

# Predictions for segmented model
pred_df$grass_mass_pred <- predict(ms1, newdata = pred_df)

ggplot(data_grass, aes(x = mean_LAI, y = grass_mass)) +
...
data_grass_log <- data_grass %>% 
group_by(grass_mass, plot_id, mean_LAI, quad_id) %>%
mutate(grass1 = grass_mass + 1)

#random intercepts model
m1 <- lmer(log(grass1) ~log(mean_LAI) + (1|plot_id), data = data_grass, REML = TRUE)
...
(ggplot(pred) + 
   geom_line(aes(x = x, y = predicted), fill = NA, color = "red") +          # slope
   geom_ribbon(aes(x = x, ymin = predicted - std.error, ymax = predicted + std.error), 
               fill = NA, alpha = 0.5) +  # error band
   geom_point(data = data_grass,                # adding the raw data
              aes(x = data_grass$mean_LAI, y = data_grass$grass_mass)) + 
