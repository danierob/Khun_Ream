...
m_simple <- lm(wind_speed ~ season * plot_type, data = kestrel)
emm_simple <- emmeans(m_simple, ~ season * plot_type)
tukey_simple <- pairs(emm_simple, adjust = "tukey")
summary(tukey_simple)

library(ggplot2)
ggplot(kestrel, aes(x = interaction(season, plot_type), y = wind_speed)) +
  geom_boxplot() +
  facet_wrap(~ plot_id)
