...
``` {r grass segmented.lme, include=FALSE }

data_season <- data_outliers %>% 
  group_by(plot_id, quad_id, LAI, season) %>% 
  summarize(grass_mass = mean(grass_mass))


m2 <- lme(grass_mass ~ LAI*season, random = ~ 1 | plot_id, data = data_season)


ms2 <- segmented.lme(m2, ~LAI, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))

summary(ms2)
confint.segmented.lme(ms2)

ms2

```

``` {r grass segmented predictions, include=FALSE}
...
fixed_effects <- fixed.effects(ms2)
...
lai_effect <- fixed_effects[2]
seasonlate_effect <- fixed_effects[3]
u_effect <- fixed_effects[4]
breakpoint <- fixed_effects[5]
u_seasonlate_effect <- fixed_effects[6]
lai_seasonlate_effect <- fixed_effects[7]


# Calculate fitted values for each segment and season
pred_data$fit <- with(pred_data, ifelse(
  season == "early",
  ifelse(LAI < breakpoint,
         intercept + lai_effect * LAI,
         intercept + lai_effect * breakpoint + (lai_effect + u_effect) * (LAI - breakpoint)),
  ifelse(LAI < breakpoint,
         (intercept + seasonlate_effect) + (lai_effect + lai_seasonlate_effect) * LAI,
         (intercept + seasonlate_effect) + (lai_effect + lai_seasonlate_effect) * breakpoint + 
           (lai_effect + u_effect + lai_seasonlate_effect + u_seasonlate_effect) * (LAI - breakpoint))
))

```

```{r grass segmented plot}
p1 <- ggplot() +
  geom_point(data = data_season, aes(x = LAI, y = grass_mass, color = season), alpha = .7) +
  geom_line(data = pred_data, aes(x = LAI, y = fit, color = season), size = 1) +
  geom_vline(xintercept = breakpoint, linetype = "dashed") +
  labs(x = "Canopy Cover (LAI)",  y = expression("Grass Biomass (g/m"^2*")"), color = "Time of Dry Season") +
  theme_bw()+
    theme(
