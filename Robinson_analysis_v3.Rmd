...
data_early <- data_season %>% 
  filter(season == "early")

data_late <- data_season %>% 
  filter(season == "late")
...
CI_fle <- confint.segmented.lme(ms_gre)
CI_fll <- confint.segmented.lme(ms_grl)
...
p1 <- ggplot() +
  geom_point(data = data_season, aes(x = LAI, y = grass_mass, color = season), alpha = .7) +
  ...
  geom_errorbarh(aes(xmin = CI_fle[1,4], xmax = CI_fle[2,4], y = max(data_season$grass_mass) * 1.05),
                 height = 15, color = "salmon", size = .7) +
  geom_errorbarh(aes(xmin = CI_fll[1,4], xmax = CI_fll[2,4], y = max(data_season$grass_mass) * 1.1),
                 height = 15, color = "cyan3", size = .7) +
  labs(x = "Canopy Cover (LAI)",  y = expression("Grass Biomass (g/m"^2*")")) +
  theme_bw()+
  theme(
...
p2 <- ggplot(data = lit_dat, aes(x = LAI, y = yvar, color = season, linetype = season)) +
  geom_line(size = .8) +
...
p2 <- p2 + geom_point(data = fine_data, aes(x =LAI, y = litter_mass, color = season), alpha = .7) +  
  theme(
    axis.title = element_text(size = 12),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
...
#model with squared tranformation
...
#make ggplot with emmip data
p3 <- ggplot(data = fine_dat, aes(x = LAI, y = yvar, color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=UCL, ymin=LCL), alpha = .3, size = .3) +
  labs(x="Canopy Cover (LAI)", y = expression("Fine Fuel Biomass (g/m"^2*")"), color = "Season", fill = "Season")+
  theme_bw()


#add points from the original data
p3 <- p3 + geom_point(data = fine_data, aes(x =LAI, y = fine_mass, color = season), alpha = .7) + theme(
    axis.title = element_text(size = 12),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
    plot.title = element_text(size = 12),
...
#make ggplot with emmip data
p4 <- ggplot(data = bulk_dat, aes(x = LAI, y = (yvar^2), color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=UCL^2, ymin=LCL^2), alpha = .3, size = .3) +
  labs(x="Canopy Cover (LAI)", y = expression("Fuel bed Denisty (g/m"^3*")"), color = "Season", fill = "Season")+
  theme_bw()

#add points from the original data
p4 <- p4 + geom_point(data = bulk_seasons, aes(x = LAI, y = bulk_density, color = season), alpha = .5) +
  theme(
    axis.title = element_text(size = 12),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
...
p3 <- ggplot() +
  ...
  labs(x="Canopy Cover (LAI)", y = "Fireline Intenisty (kW/m)kW/m)", color = "Time of Dry Season", shape = "Time of Dry Season")+
   theme_bw()
