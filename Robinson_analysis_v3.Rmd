...
p2 <- ggplot(data = lit_dat, aes(x = LAI, y = yvar, color = season)) +
...
p2 <- p2 + geom_point(data = fine_data, aes(x =LAI, y = litter_mass, color = season), alpha = .7) + theme(
    axis.title = element_text(size = 12),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
    plot.title = element_text(size = 12),
...
#add points from the original data
p2 <- p2 + geom_point(data = data, aes(x =LAI, y = litter_mass, color = season), alpha = .7) + theme(
    axis.title = element_text(size = 12),   
    legend.title = element_text(size = 10),     
    legend.text = element_text(size = 8),        
    plot.title = element_text(size = 12),
...
p2 <- ggplot(data = grassfmc_dat, aes(x = LAI, y = (yvar), color = season)) +
  geom_line(size = .8) + geom_ribbon(aes(ymax=UCL, ymin=LCL), alpha = .3, size = .3) +
  labs(x=element_blank(), y = "Grass FMC (%)" , color = "Season", fill = "Season")+
  theme_bw()
...
```{r ROS segmented}
m_ros <- lme(ROS ~ LAI*season, random = ~ 1 | plot_id, data = behave)

ms_ros <- segmented.lme(m_ros, ~LAI, z.psi = ~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
# Extract fixed effects
fixed_effects <- fixed.effects(ms_ros)
...
seasonlate_effect <- fixed_effects[3]
u_effect <- fixed_effects[4]
breakpoint_early <- fixed_effects[5] 
breakpoint_late <- fixed_effects[5] + fixed_effects[6]
u_seasonlate_effect <- fixed_effects[7]
lai_seasonlate_effect <- fixed_effects[8]
...
pred_data$fit <- with(pred_data, ifelse(
  season == "early",
  ifelse(LAI < breakpoint_early,
         intercept + lai_effect * LAI,
         intercept + lai_effect * breakpoint_early + (lai_effect + u_effect) * (LAI - breakpoint_early)),
  ifelse(LAI < breakpoint_late,
         (intercept + seasonlate_effect) + (lai_effect + lai_seasonlate_effect) * LAI,
         (intercept + seasonlate_effect) + (lai_effect + lai_seasonlate_effect) * breakpoint_late + 
           (lai_effect + u_effect + lai_seasonlate_effect + u_seasonlate_effect) * (LAI - breakpoint_late))
))



p1 <- ggplot() +
  geom_point(data = behave, aes(x = LAI, y = ROS, color = season)) +
  geom_line(data = pred_data, aes(x = LAI, y = fit, color = season), size = 1) +
  labs(
    x = element_blank(),
    y = "Rate of Surface Fire Spread (m/min)",
...
confint.segmented.lme(ms_ros)

```

```{r Flame Length segmented}
##segmented FL ----
m_fl <- lme(flame_length ~ LAI*season, random = ~ 1 | plot_id, data = behave)


ms_fl <- segmented.lme(m_fl, ~LAI, psi = c(2.46), z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))

summary(ms_fl)

# Create dataframes for prediction
pred_data_early<- data.frame(LAI = seq(0,7,.05), season = "early")
pred_data_late <- data.frame(LAI = seq(0,7,.05), season = "late")
...
# Extract fixed effects
fixed_effects <- fixed.effects(ms_fl)
...
p2 <- ggplot() +
  geom_point(data = behave, aes(x = LAI, y = flame_length, color = season)) +
  geom_line(data = pred_data, aes(x = LAI, y = fit, color = season), size = 1) +
  labs(
    x = element_blank(),
    y = "Flame Length (m)",
...
confint.segmented.lme(ms_fl)
```

```{r surface fire intensity segmented}

###intensity----
m_int <- lme(intensity ~ LAI*season, random = ~ 1 | plot_id, data = behave)


ms3 <- segmented.lme(m_int, ~LAI, psi = 2.46, z.psi =~season, x.diff=~season, random = list(plot_id=pdDiag(~1+LAI+U+G0)))
...
# Extract fixed effects
fixed_effects <- fixed.effects(ms3)
...
#plot with interaction
p3 <- ggplot() +
  geom_point(data = behave, aes(x = LAI, y = intensity, color = season)) +
  geom_line(data = pred_data, aes(x = LAI, y = fit, color = season), size = 1) +
  labs(
    x = element_blank(),
    y = "Surface Fire Intensity (kW/m)",
    color = "Time of Dry Season"
  ) +
  theme_bw()
