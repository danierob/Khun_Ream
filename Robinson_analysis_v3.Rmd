...
```{r wind speed no ddf2}
kestrel$plot_id <- factor(kestrel$plot_id)

kestrel_nodd2 <- kestrel %>%
  filter(plot_id != "ddf2") %>% 
  mutate(plot_id = droplevels(plot_id))

m_wind <- lmer(wind_speed ~ plot_type * season + (1 | plot_id), data = kestrel_nodd2)
...
m <- lmer(mean_air_temp ~ season*plot_type + (1|plot_id), data = TOMST_seasons)
emm <- emmeans(m, ~season*plot_type)
tukey_results <- pairs(emm, adjust = "tukey")
summary(tukey_results)
