...
model_early <- lm(ROS ~ LAI, data=behave_early)

model_late <- lm(ROS ~ LAI, data=behave_late)
...
```{r anova transformations}
#boxcox
library(MASS)
m <- aov(ROS+.02 ~ season*plot_type, data = fuel_stats)

# Perform Box-Cox transformation
boxcox_result <- boxcox(m, lambda = seq(-2, 2, 0.1))
...
fuel_stats$ROS_bc <- (fuel_stats$ROS^lambda_opt - 1) / lambda_opt


# Fit the linear model with the transformed response
m_bc <- aov(ROS_bc ~ season *plot_type, data = fuel_stats)
summary(transformed_lm_model)




# Log-transform the response
fuel_stats$tran_ROS <- -1/(fuel_stats$ROS)

# Fit the two-way ANOVA model with interaction
m_log <- aov(log_ROS ~ season * plot_type, data = fuel_stats)
summary(m_log)


# Check normality of residuals
par(mfrow = c(1, 2))
plot(m_log, which = 2) # Q-Q plot
shapiro.test(residuals(m_sqrt))
hist(fuel_stats$log_ROS)

plot(m_aov)

# Check homoscedasticity
plot(anova_model, which = 1) # Residuals vs Fitted plot
leveneTest(response ~ factor1 * factor2, data = data)
...
m_noint <- aov(ROS ~ season + plot_type, data = fuel_stats)

TukeyHSD(m_noint)


#significance of closed canopy ROS when open canopy is excluded
closed_stats <- fuel_stats %>% 
  filter(plot_type == "evg")

m_evg <- aov(ROS ~ season, data = closed_stats)
TukeyHSD(m_evg)

#Full model with random effects
m_full <- lmer(grass_mass ~ season*plot_type + (1|plot_id), data = fuel_stats)
summary(m_full)

emms <- emmeans(m_full, ~ season*plot_type)

# Perform Tukey's HSD test
pairs <- contrast(emms, method = "tukey")
summary(pairs)

#compare with lm grass model
m <- aov(grass_mass ~ season*plot_type, data = fuel_stats)
summary(m)
TukeyHSD(m)

#the full model has similar estimates but has different significance than the model without random effects. The m_aov model makes more intuitive ecological sense (e.g. late ddf and evg are different in the aov model). I am afraid that including random effect for plot_id is throwing the data off (same as with TOMST temp) because there are not enough samples per plot.
```
